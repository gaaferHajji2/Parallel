//The Same of ex14.c but here we parallel the operation.
#include <iostream>
#include <stdio.h>
#include <stdlib.h>
#include <limits>
...
int main(int argc, char* argv[]){
	...
	printf("My Rank is: %d, #Procs: %d\n", rank, nproc);
	double radius0[n];
	double C0[n][d];
	//Generate Data on master node
	if(rank == 0){
		for(int i=0; i<n; ++i){
			radius0[i] = get_rand(1, 5);
			for(int j=0; j<d; ++j){
				C0[i][j] = get_rand(-20, 20);
			}
		}
	}
	...
	if(rank == 0){
		for(int i=0; i<e; ++i){
			for(int j=0; j<d; ++j){
				samples[i][j] = get_rand(bb[j][0], bb[j][1]);
			}
		}
	}
	...
	for(int i=0; i<e; ++i){
		for(int j=begin; j<begin + loc_n; ++j){
			if(distance2(samples[i], C[j]) < radius[j] * radius[j])
				hit[i]= true;
		}
	}
